...
def search_files(folder_path, extensions, keywords, output_folder, output_file):
    # Excelファイルの作成
    ...
    sheet.cell(row=1, column=4).value = "キーワード"
    sheet.cell(row=1, column=5).value = "行の値"
    row_count = 2
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    for line_num, line in enumerate(lines, start=1):
                        ...
                        if hitKeywords:
                            ...
                            sheet.cell(row=row_count, column=4).value = ' '.join(hitKeywords)
                            sheet.cell(row=row_count, column=5).value = line
                            row_count += 1
...
keywords = ["John","Jane"]  # 検索するキーワード
output_folder = r"C:\myworkspace\python\search_files"  # 出力先フォルダパス
output_file = "output.xlsx"  # 出力ファイル名

search_files(folder_path, extensions, keywords, output_folder, output_file)
